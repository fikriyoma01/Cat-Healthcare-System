...
const Grooming = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editing) {
      try {
        const res = await axios.put(`/api/grooming/${currentId}`, { date, time, quota });
      ...
      try {
        const res = await axios.post('/api/grooming', { date, time, quota });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`/api/grooming/${id}`);
  ...
  return (
    <Container>
      <Box sx={{ mt: 4, mb: 2 }}>
        ...
        <Typography variant="h5" gutterBottom>Scheduled Grooming</Typography>
